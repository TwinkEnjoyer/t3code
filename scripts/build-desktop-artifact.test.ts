...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "T3 Code Delegation (Alpha)");
    assert.equal(
      resolveDesktopProductName("0.0.17-nightly.20260413.42"),
      "T3 Code Delegation (Nightly)",
    );
  });
  ...
  it.effect("omits update feeds for pull request preview builds", () =>
    Effect.gen(function* () {
      ...
      const delegation = yield* createBuildConfig(
        "win",
        "nsis",
        "0.0.42-delegation.100",
        false,
        false,
        undefined,
        undefined,
      ...
      assert.notProperty(delegation, "publish");
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "T3 Code Delegation (Alpha) 1.2.3 Installer",
      ...
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "T3 Code Delegation", schemes: ["t3code", "t3code-dev"] },
      ]);
      assert.equal(win.appId, "com.twinkenjoyer.t3code.delegation");
      assert.equal(win.productName, "T3 Code Delegation (Alpha)");
      assert.equal(win.artifactName, "T3-Code-Delegation-${version}-${arch}.${ext}");
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    ...
    assert.deepStrictEqual(configuration, {
      appId: "com.twinkenjoyer.t3code.delegation",
      teamId: "ABC1234567",
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.twinkenjoyer.t3code.delegation</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.twinkenjoyer.t3code.delegation");
