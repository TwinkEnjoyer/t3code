...
it.effect("always grants pull requests and delegation, then gates device access", () =>
  Effect.gen(function* () {
    ...
    expect(yield* capabilitiesOf(withPreview)).toEqual(["delegation", "preview", "pull-requests"]);
    expect(yield* capabilitiesOf(withoutPreview)).toEqual(["delegation", "pull-requests"]);
    expect(yield* capabilitiesOf(withDevice)).toEqual(["delegation", "device", "pull-requests"]);
  }),
);
